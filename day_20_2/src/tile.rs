...
impl Tile {
    pub fn new(input: &TilePrimitive) -> Tile {
        let insides = [false; (TILE_WIDTH - 2) * (TILE_HEIGHT - 2)];
        for (src, dst) in input
            .chunks_exact(TILE_WIDTH)
            .skip(1)
            .zip(insides.chunks_exact_mut(TILE_WIDTH - 2))
        {
            dst.copy_from_slice(&src[1..TILE_WIDTH - 2]);
        }

        Tile {
            sides: TileSides::new(input),
            insides,
        }
    }
}
...
impl TileSides {
    fn new(bool_string: &TilePrimitive) -> TileSides {
        let mut north: u16 = 0;
